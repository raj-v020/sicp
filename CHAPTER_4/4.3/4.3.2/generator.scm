    ...
    (lambda (env succeed fail)
      (define (try-next choices)
        (let ((n (length choices)))
          (if (null? choices)
            (fail)
            (let ((proc (list-ref choices (random n))))
              (proc
               env
               succeed
               (lambda () (try-next (remove proc choices))))))))
      (try-next cprocs))))
...
(define (driver-loop)
  ...
  (ambeval '(begin (define (require p) (if (not p) (amb)))
                   ...
                   (define nouns '(student professor cat class rat mat man woman boy girl person))
                   (define verbs '(studies lectures eats sleeps makes creates rests works jumps chops cuts runs sits talks))
                   (define articles '(the a))
                   (define prepositions '(for to in by with because but))
                   ...
                   #|(define (generate-simple-verb-phrase)
                     (amb (list (generate-word verbs))
                          (list (generate-word adverbs)
                                (generate-word verbs))))|#
             
                   (define (generate-verb-phrase)
                     (define (maybe-extend verb-phrase)
                       (amb (if (list? verb-phrase) verb-phrase (list verb-phrase))
                            (maybe-extend
                             (append (if (list? verb-phrase) verb-phrase (list verb-phrase))
                   ...
                   (define (generate-word word-list)
                     (if (null? word-list) (amb)
                         (let ((word (ramb (car word-list) (generate-word (cdr word-list)))))
                           (display word-list) (newline)
                           (permanent-set! word-list (remove word word-list))
                           (display word-list) (newline)
                           word)))

                   (define (generate-prepositional-phrase)
                     (append (list (generate-word prepositions))
                           (generate-noun-phrase)))

                   (define (generate)
                     (let ((sent (generate-sentence))) sent)))
