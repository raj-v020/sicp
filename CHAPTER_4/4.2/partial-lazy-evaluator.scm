  ...
  (cond ((primitive-procedure? procedure)
        ...
        ((compound-procedure? procedure)
         (eval-sequence
          ...
          (extend-environment
           (extract-variables (procedure-parameters procedure))
           (list-of-args arguments (procedure-parameters procedure) env) ; changed
           (procedure-environment procedure))))
        (else (error "Unknown procedure type: APPLY"
                     procedure))))

(define (delay-it exp env)
  (list 'thunk (list exp 'lazy) env))
(define (delay-memo-it exp env)
  (list 'thunk (list exp 'lazy-memo) env))
(define (thunk? obj)
  (tagged-list? obj 'thunk))
(define (thunk-exp thunk) (car (thunk-type thunk)))
...
(define (force-it obj)
  (if (thunk? obj) (if (lazy-memo? (thunk-type obj))
...
(define (lazy? arg) (if (pair? arg) (eq? (cadr arg) 'lazy) #f))
(define (lazy-memo? arg) (if (pair? arg) (eq? (cadr arg) 'lazy-memo) #f))
...
(define (list-of-args exps params env)
  (if (no-operands? exps)
      ...
      (let ((first-arg (first-operand exps))
            (rest-args (rest-operands exps))
            (first-param (first-operand params))
            (rest-params (rest-operands params)))
        (cond ((lazy? first-param) (cons (delay-it first-arg env)
                                   (list-of-args rest-args rest-params env)))
              ((lazy-memo? first-param) (cons (delay-memo-it first-arg env)
                                        (list-of-args rest-args rest-params env)))
              (else (cons (actual-value first-arg env)
                          (list-of-args rest-args rest-params env)))))))

(define (extract-variables params)
  (if (no-operands? params)
      '()
      (let ((first (first-operand params))
            (rest (rest-operands params)))
        (if (pair? first) (cons (car first) (extract-variables rest))
            (cons first (extract-variables rest))))))
